...
let copyUser =  {...user} // Spread оператор (позволяет забрать все свойства из объекта) - это объект ПОВЕРХНОСТНЫЙ (без вложенных элементов)
copyUser.friends.push('Helga')

// Глубокая копия, включая вложенный массив
let deepCopy = {...user, friends: [...user.friends]} //Создается новый массив внутри объекта (НЕ является ссылкой на прошлый массив)


// Создаем дефолтный массив (Родительский)
const students = [
    {name: 'Alex'},
    {name: 'Bob'},
    {name: 'Donald'},
...
let copeSt = [...students] // Поверхностная копия объекта (без вложенных элементов)
let deepCopySt = students.map((y) => ({...y})) // Возвращает новый массив (глубокая копия массива, включая все вложенности)
